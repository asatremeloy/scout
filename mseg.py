...
def panel_upgrade_calc(panel_input_file, json_results):
    ...
    is not required (or a load/space control solution is sufficient)
    given a low/med/high EE level for the upgrade measure. """

    # Read in panel solution shares json file
    with open(panel_input_file) as f:
        panel_solution_shares = json.load(f)

    # Define impacted building type, fuel type, end use, and tech type
    fuel = 'natural gas'
    end_use = 'heating'
    ...
    def multiply_dict(d, factor):
        return {k: v * factor if isinstance(v, (int, float)) else
                multiply_dict(v, factor) if isinstance(v, dict) else
                v for k, v in d.items()}

    # Function to process a single building type within a census division
    def process_bldg_type(bldg_data, region_shares):
        if fuel in bldg_data and end_use in \
            bldg_data[fuel] and 'supply' in \
                bldg_data[fuel][end_use]:
            ...
            if isinstance(supply_data, dict):
                for tech, tech_data in supply_data.items():
                    ...
                    for upgrade_level, upgrade_shares in region_shares.items():
                        new_supply_data[tech][upgrade_level] = {}
                        for panel_option, share in upgrade_shares.items():
                            new_supply_data[tech][upgrade_level][
                                panel_option] = {
                                'stock': multiply_dict(
                                    tech_data.get('stock', {}), share),
                                'energy': multiply_dict(
                                    tech_data.get('energy', {}), share)}
